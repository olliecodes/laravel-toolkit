...
use OllieCodes\Toolkit\Identity\Concerns\HasMappableIdentity;
...
class QueryBuilder extends EloquentBuilder
{
    ...
    public function findOrIdentify($id, array $columns = ['*'])
    {
        ...
        return $model ?? $this->whereKey($id)->first($columns);
    }
    ...
    {
        if (in_array(HasMappableIdentity::class, class_uses($model), true)) {
